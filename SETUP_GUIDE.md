...
#### Secret ⑨: RAPIDAPI_KEY（オプション）

1. **「New repository secret」** をクリック
2. **Name** に `RAPIDAPI_KEY` と入力
3. **Secret** に 手順11.2で取得した RapidAPI のキーを貼り付け
4. **「Add secret」** をクリック

#### Secret ⑩: RAPIDAPI_HOST（オプション）

1. **「New repository secret」** をクリック
2. **Name** に `RAPIDAPI_HOST` と入力
3. **Secret** に `twitter154.p.rapidapi.com` などのAPIホスト名を貼り付け
4. **「Add secret」** をクリック
...
3. **「Run workflow」** → **「Run workflow」** をクリック
4. `#電波人間Twitter通知` チャンネルにエラーがなく実行されることを確認
